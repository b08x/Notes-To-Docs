...
import { ArrowUpTrayIcon, SparklesIcon, CpuChipIcon, ChatBubbleBottomCenterTextIcon } from '@heroicons/react/24/outline';
...
export const InputArea: React.FC<InputAreaProps> = ({ onGenerate, isGenerating, disabled = false }) => {
  ...
  return (
    <div className="w-full max-w-4xl mx-auto perspective-1000 space-y-6">
      ...
      <div className="w-full max-w-2xl mx-auto relative group z-20">
        ...
        <div className="relative flex items-center bg-zinc-900/80 backdrop-blur-md border border-zinc-700/50 focus-within:border-blue-500/50 rounded-lg px-4 py-3 shadow-lg transition-all">
            <ChatBubbleBottomCenterTextIcon className="w-5 h-5 text-zinc-500 mr-3 shrink-0" />
            <input 
                type="text"
                value={prompt}
                onChange={(e) => setPrompt(e.target.value)}
                placeholder="Add instructions... e.g., 'Focus on the hardware reset steps'"
                className="flex-1 bg-transparent border-none outline-none text-zinc-100 placeholder-zinc-500 text-sm md:text-base focus:ring-0"
            ...
            <div className="relative z-10 flex flex-col items-center text-center space-y-6 md:space-y-8 p-6 md:p-8 w-full">
                ...
                <div className="space-y-2 md:space-y-4 w-full max-w-3xl">
                    ...
                    <p className="text-zinc-500 text-xs sm:text-base md:text-lg font-light tracking-wide">
                        ...
                        <span className="md:hidden">Tap</span> to upload file
                    </p>
                </div>
            </div>
